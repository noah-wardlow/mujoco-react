...
usePolicy(config: {
  ...
  onAction: (obs: Float32Array | Float64Array | number[], model: MujocoModel, data: MujocoData) => void;
}): {
  start: () => void;
  stop: () => void;
  isRunning: boolean;
...
| `onObservation` | `(model, data) => Float32Array \| Float64Array \| number[]` | Build observation vector from simulation state |
