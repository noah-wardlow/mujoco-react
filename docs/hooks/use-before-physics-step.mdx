...
useBeforePhysicsStep(
  callback: (input: { model: MujocoModel; data: MujocoData }) => void
): void
```
...
// Component B: typed actuator control
const shoulder = useCtrl(ModelActuators.franka.actuator1);

useBeforePhysicsStep(({ data }) => {
  shoulder.write(computeControl(data.time));
});
```
