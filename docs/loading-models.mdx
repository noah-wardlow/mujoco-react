...
|-------|------|-------------|
| `modelId` | `string` | Directory name in the model repository (e.g. `'franka_emika_panda'`) |
...
By default, models are fetched from the [MuJoCo Menagerie](https://github.com/google-deepmind/mujoco_menagerie) on GitHub. The `modelId` maps to a directory in the repository:
