...
For use inside your own `<Canvas>`. Accepts the same physics props as `MujocoCanvas` (`config`, `paused`, `speed`, `gravity`, etc.) but doesn't create a Canvas — you bring your own. This gives you full control over gl settings, post-processing pipelines, and R3F context composition.
...
The core library has **no built-in control logic**. Controllers are React components you write (or use from the library) that call `useBeforePhysicsStep` to drive `data.ctrl`. The library ships `<IkController>` as one example, but the real pattern is: write your own.
