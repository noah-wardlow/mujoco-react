...
Physics context provider designed for use inside your own React Three Fiber `<Canvas>`. Same physics configuration as [`<MujocoCanvas>`](/components/mujoco-canvas), but without the Canvas wrapper — giving you full control over gl settings, post-processing, and R3F context composition.
...
import { MujocoProvider, MujocoPhysics, SceneRenderer } from 'mujoco-react';
