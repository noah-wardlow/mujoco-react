...
`geomName` is optional and defaults to `${name}_geom` for generated
`SceneObject` entries. Set it explicitly when you need stable contact telemetry
or camera-capture filters such as `hiddenGeomNames`.
